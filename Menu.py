...
class Menu():
    def __init__(self, jsk, lcd, ipcon, board):
        ...
        ## REGISTER CALLBACKS
        self.jsk.register_callback(self.jsk.CALLBACK_PRESSED,  self.cb_pressedjsk )
        self.jsk.register_callback(self.jsk.CALLBACK_RELEASED, self.cb_releasedjsk)
        
        self.jsk.set_debounce_period(400) # Get threshold callbacks with a debounce time of 0.2 seconds (200ms)
        self.jsk.register_callback(self.jsk.CALLBACK_POSITION_REACHED, self.cb_reachedjsk) # Register threshold reached callback to function cb_reached
    ...
    def cb_pressedlcd(self, i): # Callback functions for button status LCD20X4
        ...
        if i == 0 :
            # Turn backlight on
            self.lcd.backlight_off() if self.lcd.is_backlight_on() == True else self.lcd.backlight_on()
